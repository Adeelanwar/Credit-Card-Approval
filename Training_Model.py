...
def GridTrainModel(filename = 'crx.data', test_size = 0.3, model = 'lgr', solver = 'lbfgs', max_iter = 10000):
    ...
    #Fitting a logistic regression model to the train set with Grid searching to make the model perform better
    lgr = LogisticRegression(solver = solver,max_iter=max_iter)
    params = {'penalty': ['l2'], 'C': [0.00001,0.0001,0.001,0.01,.1,1,10,100,1000]}
    TM = GridSearchCV(lgr,param_grid=params)
    TM.fit(X_train, y_train.ravel())
        
    # save the model for UI
    filename = 'Data\\' + 'lgr' + '.sav'
...
def TrainModel(filename = 'crx.data', test_size = 0.3, solver = 'lbfgs', max_iter = 10000):
