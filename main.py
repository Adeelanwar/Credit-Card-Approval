...
labeloptimizationoption = tk.Label(Model, text='Train the Model: ')
labeloptimizationoption.config(font=('helvetica', 10))
canvas1.create_window(310, 280, window=labeloptimizationoption)
optimizationoptions = ["With GridSearch", "Without GridSearch"]
optimizationoption = tk.StringVar()
optimizationoption.set(optimizationoptions[0])
optimizationoptionmenu = tk.OptionMenu(Model, optimizationoption, *optimizationoptions)
canvas1.create_window(450, 280, window=optimizationoptionmenu)
...
def Train():
    ...
    if(optimizationoption.get() == optimizationoptions[1]):
        score = TrainModel(filename = filename, max_iter = max_iter, test_size = test_size)
    else:
        score = GridTrainModel(filename = filename, max_iter = max_iter, test_size = test_size)
...
canvas1.create_window(390, 320, window=buttontrain)
